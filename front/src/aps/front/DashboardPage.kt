                    ...
                    jshit.diva(json("className" to "col-sm-6"),
                        section(json(
                            ...
                            "items" to __await<dynamic>(jshit.utils.runa({
                                ...
                                val res = try { __await(GetLiveStatusRequest().rpc(ui))
                                } catch (e: Throwable) {
                                ...
                                fun addMetric(value: String, tame: String, url: String, title: String, noStateContributions: Boolean = false) {
                                    if (value != "0") {
                                        items.push(jshit.diva(json("controlTypeName" to "addMetric", "tame" to tame, "noStateContributions" to noStateContributions, "style" to json("position" to "relative", "overflow" to "hidden")),
                                            ...
                                            ui.urlLink(json("tamy" to true, "style" to json("background" to Color.WHITE, "paddingRight" to 8, "color" to jshit.BLACK_BOOT), "blinkOpts" to json("dwidth" to -8),
                                ...
                                res.case.let{when(it) {
                                    is GetLiveStatusRequest.Response.Case.ForAdmin -> {
                                        addMetric(value = it.profilesToApprove, tame = "profilesToApprove", url = "admin-users.html?filter=2approve", title = t("TOTE", "Профилей зааппрувить"))
                                        addMetric(value = it.suka, tame = "suka", noStateContributions = true, url = "suka.html", title = t("TOTE", "Сцуко-метрика"))
                                    }
                                    is GetLiveStatusRequest.Response.Case.ForWriter -> {
                                        addMetric(value = it.suka, tame = "suka", noStateContributions = true, url = "suka.html", title = t("TOTE", "Сцуко-метрика"))
                                    }
                                    is GetLiveStatusRequest.Response.Case.ForCustomer -> {
                                        addMetric(value = it.suka, tame = "suka", noStateContributions = true, url = "suka.html", title = t("TOTE", "Сцуко-метрика"))
                                    }
                                }}
