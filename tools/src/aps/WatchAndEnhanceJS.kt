...
fun watch() {
    ...
    val dir = Paths.get("${const.file.APS_HOME}/front/out")
...
fun test() {
    println("Testing...")
    val jsFileEntry = Entry("${const.file.APS_HOME}/front/out/front.js")
    val mapFileEntry = Entry("${const.file.APS_HOME}/front/out/front.js.map")
    doStuff(jsFileEntry, mapFileEntry)
}
...
//            }
    ...
    inMapFile.copyTo(File("${const.file.APS_HOME}/aps/built/ua-writer/kotlin/" + inMapFile.name), overwrite = true)

    File("${const.file.APS_HOME}/aps/built/ua-writer/kotlin/" + inJSFile.name.replace(Regex("\\.js$"), "-enhanced.js")).run{
