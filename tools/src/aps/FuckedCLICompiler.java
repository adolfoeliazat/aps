...
public abstract class FuckedCLICompiler<A extends CommonCompilerArguments> {
    ...
    private A parseArguments(@NotNull PrintStream errStream, @NotNull MessageRenderer messageRenderer, @NotNull String[] args) {
        try {
            ...
            errStream.println("No fucking usage");
            // Orig: Usage.print(errStream, createArguments(), false);
        }
        catch (Throwable t) {
    ...
    private ExitCode exec(
        ...
        FuckedK2JVMCompiler.Companion.resetInitStartTime();
        ...
        if (arguments.help || arguments.extraHelp) {
            errStream.println("No fucking usage");
            // Orig: Usage.print(errStream, createArguments(), arguments.extraHelp);
            return OK;
        }
        ...
        try {
            ...
            if (!EnoughFuckedCompiling.Companion.getWasCreated()) {
     ...
     */
    public static void doMain(@NotNull FuckedCLICompiler compiler, @NotNull String[] args) {
    ...
    @NotNull
    public static ExitCode doMainNoExit(@NotNull FuckedCLICompiler compiler, @NotNull String[] args) {
        try {
